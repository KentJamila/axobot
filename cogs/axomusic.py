...
class Music:
    ...
    async def summon(self, ctx):
        ...
        if summoned_channel is None:
            await self.bot.say('`Are you sure youre in a channel?`')
            return False
    ...
    async def play(self, ctx, *, song : str):
        ...
        if state.voice is None:
            ...
            await self.bot.say(":mag_right: `searching, please wait...`")
            if not success:
                return
        ...
        try:
            ...
            await self.bot.send_message(ctx.message.channel, "`Queued`" + str(entry))
    ...
    async def volume(self, ctx, value : int):
        ...
        if state.is_playing():
            ...
            await self.bot.say(':level_slider: `Volume is now {:.0%}`'.format(player.volume))
    ...
    async def resume(self, ctx):
        ...
        if state.is_playing():
            player = state.player
            player.resume()
            await self.bot.say(':musical_note: `Resumed`')
    ...
    async def pause(self, ctx):
        ...
        if state.is_playing():
            player = state.player
            player.pause()
            await self.bot.say(':pause_button: `Music Paused`')
    ...
    async def skip(self, ctx):
        ...
        if not state.is_playing():
            await self.bot.say('Not playing any music right now - Use `?play`')
            return
    ...
    async def songinfo(self, ctx):
        ...
        if state.current is None:
            await self.bot.say('Not playing anything - Use `?play`')
        else:
